# Generated by Powerlevel10k configuration wizard on 2020-08-30 at 19:13 CEST.
...
() {
  ...
  ### Maven ###
  function prompt_maven_ctx() {
    # Cache: git_root -> 1 (maven) or 0 (not maven)
    typeset -gA _P10K_MAVEN_REPO_CACHE

    local d=$PWD
    local marker_root=""

    # 1) Fast path: find nearest Maven root marker by walking up.
    while [[ $d != / ]]; do
      if [[ -f $d/pom.xml || -d $d/.mvn || -f $d/mvnw ]]; then
        marker_root=$d
        break
      fi
      d=${d:h}
    ...
    # Render: M at project root, m in subdirs.
    if [[ $PWD == $marker_root ]]; then
      p10k segment -t 'M'
    else
      p10k segment -t 'm'
    fi
  ...
  typeset -g POWERLEVEL9K_KUBECONTEXT_SHOW_ON_COMMAND='kubectl|helm|kubens|kubectx|oc|istioctl|kogito|k9s|helmfile|sky'
...
'builtin' 'unset' 'p10k_config_opts'
  ...
  ###############################[ maven_ctx: Maven context ]#################################
  # Match java_version styling for a seamless look.
  typeset -g POWERLEVEL9K_MAVEN_CTX_FOREGROUND=1
  typeset -g POWERLEVEL9K_MAVEN_CTX_BACKGROUND=7
  # No icon by default.
  typeset -g POWERLEVEL9K_MAVEN_CTX_VISUAL_IDENTIFIER_EXPANSION=